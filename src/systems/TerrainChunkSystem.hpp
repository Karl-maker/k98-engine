...
public:
    void update(Registry& registry, Vec3 playerPos, TerrainHeightField* heightField, const TerrainWorldMap* worldMap = nullptr)
    {
        ...
        for (int x = -radius; x <= radius; ++x) {
            for (int z = -radius; z <= radius; ++z) {
                ...
                if (!chunkExists(registry, cx, cz))
                    createChunk(registry, cx, cz, settings, heightField, stride, cellSize, worldMap);
            }
        }
...
private:
        ...
        const TerrainWorldMap* worldMap)
    {
        ...
        fillHeightMap(hm, cx, cz, stride, cellSize, settings, worldMap);
